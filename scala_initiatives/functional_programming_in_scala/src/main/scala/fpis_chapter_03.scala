...
object FPISExerciseChapter03 extends ScalaInitiativesExercise {
  ...
  // ???: Weird identation.
        ...
        object Tree {
          ...
          def fold[A, B](
            t: Tree[A])(fLeaf: Leaf[A] => B)(fBranch: (B, B) => B): B = {
              t match {
                case Leaf(v) => fLeaf(Leaf(v))
                case Branch(l, r) => fBranch(
                  fold(l)(fLeaf)(fBranch),
          ...
          def mapUsingFold[A, B](t: Tree[A])(f: A => B): Tree[B] = {
            ...
            def mapFuncBranch(l: Tree[B], r: Tree[B]): Tree[B] = Branch(
              mapUsingFold(l)(identity),
              mapUsingFold(r)(identity))
