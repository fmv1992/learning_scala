...
object FPISExerciseChapter03 extends ScalaInitiativesExercise {
  ...
  object FPList {
    ...
    def apply[A](as: A*): FPList[A] =
      if (as.isEmpty) FPNil else FPCons(as.head, apply(as.tail: _*))
    ...
    def product2(ns: FPList[Double]): Double = foldRight(ns, 1.0)(_ * _)
    ...
    def foldRightWithShortCircuit[A, B](as: FPList[A], z: B, ss: B)(
        f: (A, B) => B
    ): B = {
      as match {
        case FPNil => z
    ...
    def sumFoldLeft(ns: FPList[Int]): Int = foldLeft(ns, 0)((x, y) => x + y)

    def productFoldLeft(ns: FPList[Double]): Double = foldLeft(ns, 1.0)(_ * _)

    def productFoldLeft(ns: FPList[Int]): Int = foldLeft(ns, 1)(_ * _)
  ...
  object Tree {
    ...
    def sizeUsingFold[C](t: Tree[C]): Int = {
      def sizeFuncLeaf(x: Leaf[C]) = 1
      def sizeFuncBranch(x: Int, y: Int): Int = x + y + 1
      fold(t)(sizeFuncLeaf)(sizeFuncBranch)
    }

    def maxUsingFold(t: Tree[Int]): Int = {
      def maxFuncLeaf(x: Leaf[Int]) = x.value
      def maxFuncBranch(x: Int, y: Int): Int = x max y
      fold(t)(maxFuncLeaf)(maxFuncBranch)
    }

    def depthUsingFold[A](t: Tree[A]): Int = {
      def depthFuncLeaf(x: Leaf[A]) = 1
      def depthFuncBranch(x: Int, y: Int): Int = (x max y) + 1
      fold(t)(depthFuncLeaf)(depthFuncBranch)
    }

    def mapUsingFold[A, B](t: Tree[A])(f: A => B): Tree[B] = {
      def mapFuncLeaf(x: Leaf[A]): Tree[B] = Leaf(f(x.value))
      def mapFuncBranch(l: Tree[B], r: Tree[B]): Tree[B] =
        Branch(mapUsingFold(l)(identity), mapUsingFold(r)(identity))
      fold(t)(mapFuncLeaf)(mapFuncBranch)
    }
